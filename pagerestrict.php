<?php
/*
Plugin Name: Page Restrict for EAGLE
...
function pr_get_page_content() {
	...
	if ( pr_get_opt ( 'loginform' ) == true ) :
		...
		' . $errors . '
			<p>
				<label for="log">' . apply_filters( 'pr_username_label', 'Username' ) . ' <input type="text" name="log" id="log" value="' . wp_specialchars ( stripslashes ( $user_login ) , 1 ) . '" size="22" /></label><br />
				<label for="pwd">' . apply_filters( 'pr_password_label' , 'Password' ) . ' <input type="password" name="pwd" id="pwd" size="22" /></label><br /><br />
				<input type="submit" name="submit" value="Log In" class="button" />
				<label for="rememberme"><input name="rememberme" id="rememberme" type="checkbox" value="forever" /> Remember me</label><br />
			</p>
		...
		// $pr_page_content .= '<a href="' . get_bloginfo ( 'wpurl' ) . '/wp-login.php?action=lostpassword">Lost your password?</a></p>';
		$pr_page_content .= '</p>';
		global $post;
